...
/* my decode operand helper of riscv R I B J instruction*/
 ...
 /**
   * op  : opead 
   * val : eg or imm
   *     : ISAeInfo and Inr define in /include/isa/decode.h
   * lod_val : tue( load data from reg )
  */
 //op  : operand 
make_DHelper(R) {
  decode_op_r(id_src, decinfo.isa.instr.rs1, true);
...
make_DHelper(I) {
 ...
 //ISAodeInfnstr define in /include/isa/decode.h
make_DHelper(B) {
  decode_op_r(id_src, decinfo.isa.instr.rs1, true);
  decode_op_r(id_src2, decinfo.isa.instr.rs2, true);
...
//oad_val : true( load data from reg )
make_DHelper(J) {
